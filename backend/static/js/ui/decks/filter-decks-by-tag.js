...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!tagFilterButton || !tagFilterDropdown || !tagFilterOptionsContainer) {
        console.warn("Deck tag filter UI elements (button, dropdown, options) not found.");
        return;
    }
    ...
    async function populateTagFilter() {
        try {
            ...
            tags.forEach(tag => {
                ...
                div.innerHTML = `
                    <input id="${checkboxId}" name="deck_tag_filter" type="checkbox" value="${tag.id}" class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
    ...
    tagFilterButton.addEventListener('click', toggleDropdown);

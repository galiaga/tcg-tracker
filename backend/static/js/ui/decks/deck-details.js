...
async function quickLogMatch(deckId, resultValue) {
    ...
    try {
        const csrfToken = await getCsrfToken(); // CSRF needed for POST
        ...
        const data = await response.json();
        if (response.ok) {
            if (typeof showFlashMessage === 'function') showFlashMessage(`${resultText} logged successfully!`, "success");
            return true;
        } else {
...
function setupQuickLogButtonsListener() {
    ...
    container.addEventListener('click', async (event) => {
        ...
        if (!deckId || typeof resultValue === 'undefined') return;
        ...
        try {
            ...
            if (typeof loadDeckMatches === 'function') loadDeckMatches();
        } catch (error) {
             allButtons.forEach(btn => {
...
function renderDeckDetails(deck, container) {
    ...
    let tagPillsHtml = deck.tags?.map(tag =>
        `<span class="tag-pill inline-flex items-center gap-1 bg-violet-100 text-violet-800 text-xs font-medium px-2 py-0.5 rounded-md mr-1 mb-1" data-tag-id="${tag.id}">
            ${tag.name}
            <button type="button" class="remove-tag-button ml-0.5 text-violet-500 hover:text-violet-700 font-bold focus:outline-none" aria-label="Remove tag ${tag.name}">×</button>
        </span>`
    ).join('') || '';
    ...
    const tagsContainerHtml = `<div class="mt-4 pt-4 border-t border-gray-100 px-4 sm:px-6"><h3 class="text-sm font-medium text-gray-500 mb-2">Tags:</h3><div class="flex flex-wrap items-center gap-x-1">${tagPillsHtml}${addTagButtonHtml}</div></div>`;

    let winrateColorClass = 'text-gray-900';
    const winRate = deck.win_rate ?? 0;
    if (winRate > 55) winrateColorClass = 'text-green-600 font-semibold';
    else if (winRate >= 45) winrateColorClass = 'text-yellow-600 font-semibold';
    else if (winRate > 0) winrateColorClass = 'text-red-600 font-semibold';

    const statsHtml = `<div class="text-sm space-y-3 mb-4 px-4 sm:px-6"><div class="space-y-1"><div><span class="font-medium text-gray-500 mr-1 w-20 inline-block">Format:</span><span class="text-gray-900">${deck.deck_type?.name ?? 'N/A'}</span></div>${deck.commander_name ? `<div><span class="font-medium text-gray-500 mr-1 w-20 inline-block">Commander:</span><span class="text-gray-900">${deck.commander_name}</span></div>` : ""}${deck.associated_commander_name ? `<div><span class="font-medium text-gray-500 mr-1 w-20 inline-block">Associated:</span><span class="text-gray-900">${deck.associated_commander_name}</span></div>` : ""}</div><div class="border-t border-gray-100 pt-3 space-y-2"><div class="flex items-center justify-between gap-x-4"><div><span class="font-medium text-gray-500 mr-1">Winrate:</span><span class="${winrateColorClass} text-base">${winRate}%</span></div><div class="text-right flex gap-x-3"><div><span class="font-medium text-gray-500 block text-xs">Matches</span><span class="text-gray-900 block text-base font-medium">${deck.total_matches ?? 0}</span></div><div><span class="font-medium text-gray-500 block text-xs">Wins</span><span class="text-gray-900 block text-base font-medium">${deck.total_wins ?? 0}</span></div></div></div></div></div>`;
    const actionMenuHtml = `<div class="relative deck-actions flex-shrink-0"><button type="button" id="deck-detail-options-btn" class="p-2 rounded-full text-gray-500 hover:bg-gray-100 hover:text-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-violet-500" data-deck-id="${deck.id}" data-deck-name="${escapedDeckName}" aria-label="Deck options" aria-haspopup="true" aria-expanded="false"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 sm:w-6 sm:h-6 pointer-events-none"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5ZM12 12.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5ZM12 18.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5Z" /></svg></button><div id="deck-detail-action-menu" class="action-menu hidden absolute right-0 mt-2 w-48 origin-top-right bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-20" role="menu" aria-orientation="vertical"><div class="py-1" role="none"><button type="button" class="menu-rename-btn text-gray-700 group flex items-center w-full px-4 py-2 text-sm text-left hover:bg-gray-100 hover:text-gray-900" role="menuitem" data-deck-id="${deck.id}" data-current-name="${escapedDeckName}"><svg class="mr-3 h-5 w-5 text-gray-400 group-hover:text-gray-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" /></svg>Update Name</button><button type="button" class="menu-delete-btn text-red-600 group flex items-center w-full px-4 py-2 text-sm text-left hover:bg-red-50 hover:text-red-700" role="menuitem" data-deck-id="${deck.id}" data-deck-name="${escapedDeckName}"><svg class="mr-3 h-5 w-5 text-red-400 group-hover:text-red-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd" /></svg>Delete Deck</button></div></div></div>`;
    const quickLogButtonsHtml = `<div class="px-4 sm:px-6 mt-4 pt-4 border-t border-gray-100"><h3 class="text-sm font-medium text-gray-500 mb-2 text-center">Quick Log Result:</h3><div class="flex justify-center space-x-3"><button type="button" data-result="0" class="quick-log-btn bg-green-500 hover:bg-green-600 text-white font-medium py-2 px-5 rounded-lg shadow-sm transition flex-1 max-w-[100px]">Win</button><button type="button" data-result="1" class="quick-log-btn bg-red-500 hover:bg-red-600 text-white font-medium py-2 px-5 rounded-lg shadow-sm transition flex-1 max-w-[100px]">Loss</button><button type="button" data-result="2" class="quick-log-btn bg-yellow-500 hover:bg-yellow-600 text-white font-medium py-2 px-5 rounded-lg shadow-sm transition flex-1 max-w-[100px]">Draw</button></div></div>`;

    container.innerHTML = `<div class="pb-4"><div class="flex items-start justify-between pb-4 mb-2 border-b border-gray-200 px-4 sm:px-6 pt-4 sm:pt-6"><h2 id="deck-detail-name" class="text-xl sm:text-2xl font-semibold text-gray-900 min-w-0 break-words pr-3">${deck.name}</h2>${actionMenuHtml}</div>${statsHtml}${quickLogButtonsHtml}${tagsContainerHtml}</div>`;
...
async function handleTagClickEvents(event) {
    ...
    if (removeButton) {
        ...
        if (!tagId) return;
        removeButton.disabled = true; tagPill.style.opacity = '0.5';
        try {
            const csrfToken = await getCsrfToken(); // CSRF needed for DELETE tag
            const response = await authFetch(`/api/decks/${deckId}/tags/${tagId}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': csrfToken } });
            ...
            removeButton.disabled = false; if(tagPill) tagPill.style.opacity = '1';
        }
    } else if (addButton) {
        event.preventDefault(); event.stopPropagation();
...
async function loadDeckDetails(deckId) {
    ...
    try {
        ...
        container.dataset.deckId = deck.id; container.dataset.deckName = deck.name;
        ...
        if (container) {
             ...
             delete container.dataset.deckId; delete container.dataset.deckName;
         }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    try {
        ...
        if (isNaN(deckId)) throw new Error("Parsed Deck ID is NaN");
    } catch(e) {
    ...
    if (quickAddModal && quickAddModalCloseBtn && typeof closeQuickAddTagModal === 'function') {
        ...
        quickAddModal.addEventListener('click', (event) => { if (event.target === quickAddModal) closeQuickAddTagModal(); });
    }
});
...
function setupActionMenuListeners(deck) {
    ...
    optionsBtn.dataset.deckId = deck.id; optionsBtn.dataset.deckName = escapedDeckName; optionsBtn.ariaLabel = `Deck options for ${currentName}`;
    ...
    // Re-attach listener to potentially new button instance after re-render
    const newOptionsBtn = optionsBtn.cloneNode(true);
    optionsBtn.parentNode.replaceChild(newOptionsBtn, optionsBtn);
    newOptionsBtn.addEventListener('click', (event) => { event.stopPropagation(); actionMenu.classList.toggle('hidden'); });

    if (renameBtn) {
        const newRenameBtn = renameBtn.cloneNode(true);
        renameBtn.parentNode.replaceChild(newRenameBtn, renameBtn);
        newRenameBtn.addEventListener('click', () => { closeMenu(); promptForRename(deck.id, currentName); });
        newRenameBtn.dataset.currentName = escapedDeckName;
    }
    if (deleteBtn) {
         const newDeleteBtn = deleteBtn.cloneNode(true);
         deleteBtn.parentNode.replaceChild(newDeleteBtn, deleteBtn);
         newDeleteBtn.addEventListener('click', () => { closeMenu(); showDeleteConfirmation(deck.id, currentName); });
         newDeleteBtn.dataset.deckName = escapedDeckName;
    }

    // Click outside listener
    const clickOutsideListener = (event) => {
        const currentOptionsBtn = document.getElementById('deck-detail-options-btn'); // Get current instance
        ...
        if (currentActionMenu && currentOptionsBtn && !currentOptionsBtn.contains(event.target) && !currentActionMenu.contains(event.target)) {
            if (!currentActionMenu.classList.contains('hidden')) closeMenu();
        }
    };
    document.removeEventListener('click', clickOutsideListener); // Clean up previous
    ...
    async function updateDeckNameOnServer(deckId, newName) {
        try {
            const csrfToken = await getCsrfToken(); // CSRF needed for PATCH
            const response = await authFetch(`/api/decks/${deckId}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken },
    ...
    function showDeleteConfirmation(deckId, deckName) {
         // Updated confirmation message for soft delete
         if (window.confirm(`Are you sure you want to delete the deck "${deckName}"? It will be hidden but can be recovered later.`)) {
    ...
    async function deleteDeckOnServer(deckId) {
        try {
            const csrfToken = await getCsrfToken(); // CSRF needed for DELETE
            if (!csrfToken) return;
            ...
            if (response.ok) {
                ...
                if (response.status === 401 && errorData.msg === "CSRF token validation failed") {
                     if (typeof showFlashMessage === 'function') showFlashMessage("Security token mismatch. Please refresh and try again.", "danger");
                } else {
                     if (typeof showFlashMessage === 'function') showFlashMessage(`Error deleting deck: ${errorData.error || errorData.msg || response.statusText}`, "danger");
                }
            }
        } catch (error) {
            console.error('Network or Security error while deleting deck:', error);
...
if (typeof showFlashMessage === 'undefined') {
     function showFlashMessage(message, category) {
          ...
          // alert(`[${category.toUpperCase()}] ${message}`); // Optional alert fallback
     }
}
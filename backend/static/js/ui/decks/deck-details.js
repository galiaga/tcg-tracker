...
function renderDeckDetails(deck, container) {
     ...
     const tagsContainerHtml = `<div class="mt-4 pt-4 border-t border-gray-100"><h3 class="text-sm font-medium text-gray-500 mb-1">Tags:</h3><div class="flex flex-wrap items-center gap-x-1">${tagPillsHtml}${addTagButtonHtml}</div></div>`;
     container.innerHTML = `<div class="p-4 sm:p-6"><div class="flex items-start justify-between pb-4 mb-4 border-b border-gray-200"><h2 id="deck-detail-name" class="text-xl sm:text-2xl font-semibold text-gray-900 min-w-0 break-words pr-3">${deck.name}</h2><div class="relative deck-actions flex-shrink-0"><button type="button" id="deck-detail-options-btn" class="p-2 rounded-full text-gray-500 hover:bg-gray-100 hover:text-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500" data-deck-id="${deck.id}" data-deck-name="${escapedDeckName}" aria-label="Deck options" aria-haspopup="true" aria-expanded="false"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 sm:w-6 sm:h-6 pointer-events-none"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5ZM12 12.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5ZM12 18.75a.75.75 0 1 1 0-1.5.75.75 0 0 1 0 1.5Z" /></svg></button><div id="deck-detail-action-menu" class="action-menu hidden absolute right-0 mt-2 w-48 origin-top-right bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-20" role="menu" aria-orientation="vertical"><div class="py-1" role="none"><button type="button" class="menu-rename-btn text-gray-700 group flex items-center w-full px-4 py-2 text-sm text-left hover:bg-gray-100 hover:text-gray-900" role="menuitem" data-deck-id="${deck.id}" data-current-name="${escapedDeckName}"><svg class="mr-3 h-5 w-5 text-gray-400 group-hover:text-gray-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" /></svg>Update Name</button><button type="button" class="menu-delete-btn text-red-600 group flex items-center w-full px-4 py-2 text-sm text-left hover:bg-red-50 hover:text-red-700" role="menuitem" data-deck-id="${deck.id}" data-deck-name="${escapedDeckName}"><svg class="mr-3 h-5 w-5 text-red-400 group-hover:text-red-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd" /></svg>Delete Deck</button></div></div></div></div><div class="flex flex-wrap gap-x-4 gap-y-2 text-sm mb-4"><span><span class="font-medium text-gray-500 mr-1">Format:</span><span class="text-gray-900">${deck.deck_type?.name ?? 'N/A'}</span></span><span><span class="font-medium text-gray-500 mr-1">Winrate:</span><span class="text-gray-900">${deck.win_rate ?? 0}%</span></span><span><span class="font-medium text-gray-500 mr-1">Matches:</span><span class="text-gray-900">${deck.total_matches ?? 0}</span></span><span><span class="font-medium text-gray-500 mr-1">Wins:</span><span class="text-gray-900">${deck.total_wins ?? 0}</span></span>${deck.commander_name ? `<span><span class="font-medium text-gray-500 mr-1">Commander:</span><span class="text-gray-900">${deck.commander_name}</span></span>` : ""}${deck.associated_commander_name ? `<span><span class="font-medium text-gray-500 mr-1">Associated Commander:</span><span class="text-gray-900">${deck.associated_commander_name}</span></span>` : ""}</div>${tagsContainerHtml}</div>`;
    ...
    async function updateDeckNameOnServer(deckId, newName) {
        try {
            ...
            if (response.ok) {
                ...
                updatePageTitle(newName); // Actualizar título de la página también
            } else {
                const errorData = await response.json().catch(() => ({}));

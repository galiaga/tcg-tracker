...
import { formatMatchResult } from '../../utils.js'; // Assuming this utility exists and works
...
function getSelectedMatchTagIds() {
    ...
    return Array.from(optionsContainer.querySelectorAll('input[type="checkbox"]:checked'))
        ...
        .filter(value => value && value.trim() !== "" && checkbox.id !== 'select-all-match-tags-checkbox'); // Example: exclude a select-all
}

async function handleRemoveMatchTagClick(event) {
    const removeButton = event.target.closest('.remove-match-tag-button');
    if (!removeButton) return false; // Indicate not handled by this function
    ...
    try {
        ...
        if (response.ok) {
            if (tagPill) tagPill.remove(); // Remove from UI
            // if (typeof showFlashMessage === 'function') showFlashMessage(responseData.message || "Tag removed.", "success");
...
function displayMatches(matches, containerElement, noMatchesElement) {
    ...
    matches.forEach(match => {
        ...
        // Base classes, border color will be added dynamically. Using left-border for emphasis.
        ...
        let borderColorClassString = 'border-gray-300 dark:border-gray-600'; // Default left border as a string
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (quickAddModal && quickAddModalCloseBtn && quickAddModalDoneBtn && typeof closeQuickAddTagModal === 'function') {
        ...
        // It's generally better to attach keydown to the modal if it's always present,
        // or to document only when the modal is visible and remove it when hidden.
        // For simplicity, this document listener is fine if the modal is the only one using Escape this way.
